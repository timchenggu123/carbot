...
class Picarx(object):
    ...
    def __init__(self, 
        ...
        print("cali values:", self.dir_cali_val, self.cam_pan_cali_val, self.cam_tilt_cali_val)
    ...
    def update_motor(self):
        left_power = self.left_motor_base_power - self.left_motor_differential_power
        right_power = self.right_motor_base_power - self.right_motor_differential_power
        right_power =  -1*right_power
        self.set_motor_speed(1, left_power)
        self.set_motor_speed(2, right_power)
        
    def update_pan(self):
        self.set_cam_pan_angle(self.pan_angle)

    def update_tilt(self):
        self.set_cam_tilt_angle(self.tilt_angle)

    def pan_right(self, inc=5):
        self.pan_angle = min(self.CAM_PAN_MAX, self.pan_angle + inc)
        
    def pan_left(self, inc=5):
        self.pan_angle = max(self.CAM_PAN_MIN, self.pan_angle - inc)

    def tilt_up(self, inc=5):
        self.tilt_angle = min(self.CAM_TILT_MAX, self.tilt_angle + inc) 
    
    def tilt_down(self,inc=5):
        self.tilt_angle = max(self.CAM_TILT_MIN, self.tilt_angle - inc) 
        
    def accelerate(self, inc=10):
        self.left_motor_base_power += inc
        self.right_motor_base_power += inc
        self.left_motor_base_power = max(-100, self.left_motor_base_power)
        self.left_motor_base_power = min(100, self.left_motor_base_power)
    ...
    def turn(self, inc=2):
        ...
        # print((power_scale, self.left_motor_differential_power, self.right_motor_differential_power))
